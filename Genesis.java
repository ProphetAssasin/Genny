...
public boolean respond(String s) { //decide how and what to respond, return if we should keep the program alive
    ...
    String response = "";
    if(lastResponse == null) { //first message must always be a greeting
        boolean newGreeting = true;
        for(String r : fm.getResponses(ResponseType.GREETING)) {
            if(transform(s).equalsIgnoreCase(transform(r)))
            	newGreeting = false; //if this is a greeting that we know about, we dont need to store it again
        }
        if(newGreeting) //store a new greeting for use in another session (or this one)
        ...
        for(String r : fm.getResponses(ResponseType.GREETING)) { //check if THE LAST MESSAGE is another greeting
            if(transform(lastResponse).equalsIgnoreCase(transform(r)))
                notg = false;
        }
        boolean notf = true;
        ...
        if((!notf || s.equalsIgnoreCase("exit")) && notg) { //if they're doing a farewell or saying "exit", and THE LAST MESSAGE is not a greeting
            ...
            if(newFarewell) //if it's new, store it for another session (or this one)
                fm.setResponse(ResponseType.FAREWELL, removeEndPunc(lastResponse));
            //say bye back
  ...
  // added a laugh as an exception but not too sure how/if it works
    ...
    if(laugh > s.toCharArray().length / 2) { //if >50% h's or l's
        ...
        if(newLaugh) //if it's new, save it for later
    ...
    lastResponse = s; //set the new last message
